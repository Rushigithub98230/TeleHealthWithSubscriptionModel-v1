...
public class LocalFileStorageService : IFileStorageService
{
    ...
    public async Task<JsonModel> UploadFileAsync(byte[] fileData, string fileName, string contentType)
    {
        try
        {
            ...
            return new JsonModel { data = uniqueFileName, Message = "File uploaded successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error uploading file: {FileName}", fileName);
            return new JsonModel { data = new object(), Message = $"Error uploading file: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public async Task<JsonModel> DownloadFileAsync(string filePath)
    {
        try
        {
            ...
            return new JsonModel { data = data, Message = "File downloaded successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error downloading file: {FilePath}", filePath);
            return new JsonModel { data = new object(), Message = $"Error downloading file: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public async Task<JsonModel> DeleteFileAsync(string filePath)
    {
        try
        {
            ...
            return new JsonModel { data = true, Message = "File deleted successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error deleting file: {FilePath}", filePath);
            return new JsonModel { data = new object(), Message = $"Error deleting file: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public async Task<JsonModel> FileExistsAsync(string filePath)
    {
        try
        {
            ...
            return new JsonModel { data = exists, Message = exists ? "File exists" : "File not found", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error checking file existence: {FilePath}", filePath);
            return new JsonModel { data = new object(), Message = $"Error checking file existence: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public async Task<JsonModel> GetFileSizeAsync(string filePath)
    {
        try
        {
            ...
            return new JsonModel { data = fileInfo.Length, Message = "File size retrieved successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error getting file size: {FilePath}", filePath);
            return new JsonModel { data = new object(), Message = $"Error getting file size: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public Task<JsonModel> GetFileUrlAsync(string filePath)
    {
        ...
        return Task.FromResult(JsonModel.SuccessResponse(url, "File URL generated successfully"));
    }
    ...
    public async Task<JsonModel> GetFileInfoAsync(string filePath)
    {
        try
        {
            ...
            return new JsonModel { data = dto, Message = "File info retrieved successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error getting file info: {FilePath}", filePath);
            return new JsonModel { data = new object(), Message = $"Error getting file info: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    {
        var url = $"/uploads/{filePath}";
        return Task.FromResult(JsonModel.SuccessResponse(url, "Secure file URL generated successfully"));
    }
    ...
    public async Task<JsonModel> CreateDirectoryAsync(string directoryPath)
    {
        try
        {
            ...
            if (Directory.Exists(fullPath))
                return new JsonModel { data = true, Message = "Directory already exists", StatusCode = 200 };
            
            Directory.CreateDirectory(fullPath);
            return new JsonModel { data = true, Message = "Directory created successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error creating directory: {DirectoryPath}", directoryPath);
            return new JsonModel { data = new object(), Message = $"Error creating directory: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public async Task<JsonModel> DeleteDirectoryAsync(string directoryPath)
    {
        try
        {
            var fullPath = Path.Combine(_baseStoragePath, directoryPath);
            if (!Directory.Exists(fullPath))
                return new JsonModel { data = new object(), Message = "Directory not found", StatusCode = 404 };
            
            Directory.Delete(fullPath, true);
            return new JsonModel { data = true, Message = "Directory deleted successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error deleting directory: {DirectoryPath}", directoryPath);
            return new JsonModel { data = new object(), Message = $"Error deleting directory: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public async Task<JsonModel> ListFilesAsync(string directoryPath, string? searchPattern = null)
    {
        try
        {
            ...
            return new JsonModel { data = files, Message = "Files listed successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error listing files: {DirectoryPath}", directoryPath);
            return new JsonModel { data = new object(), Message = $"Error listing files: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public Task<JsonModel> ValidateFileAccessAsync(string filePath, Guid userId)
    {
        ...
        try
        {
            ...
            if (!exists)
                return Task.FromResult(JsonModel.ErrorResponse("File not found", 404));
            
            // Basic access validation - in production, implement proper access control
            // For now, if file exists, grant access
            return Task.FromResult(JsonModel.SuccessResponse(true, "Access granted"));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error validating file access: {FilePath} for user {UserId}", filePath, userId);
            return Task.FromResult(JsonModel.ErrorResponse($"Error validating file access: {ex.Message}"));
        }
    }
    ...
    public Task<JsonModel> SetFilePermissionsAsync(string filePath, FilePermissions permissions)
    {
        ...
        return Task.FromResult(JsonModel.SuccessResponse(true, "File permissions set successfully"));
    }
    ...
    public async Task<JsonModel> EncryptFileAsync(byte[] fileData, string encryptionKey)
    {
        try
        {
            ...
            return new JsonModel { data = base64Result, Message = "File encrypted successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error encrypting file data");
            return new JsonModel { data = new object(), Message = $"Error encrypting file: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public async Task<JsonModel> DecryptFileAsync(string encryptedFilePath, string encryptionKey)
    {
        try
        {
            ...
            return new JsonModel { data = decryptedData, Message = "File decrypted successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error decrypting file: {FilePath}", encryptedFilePath);
            return new JsonModel { data = new object(), Message = $"Error decrypting file: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public async Task<JsonModel> UploadMultipleFilesAsync(IEnumerable<FileUploadDto> files)
    {
        try
        {
            ...
            return new JsonModel { data = uploadedPaths, Message = "Multiple files uploaded successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error uploading multiple files");
            return new JsonModel { data = new object(), Message = $"Error uploading multiple files: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public async Task<JsonModel> DeleteMultipleFilesAsync(IEnumerable<string> filePaths)
    {
        try
        {
            ...
            return new JsonModel { data = allDeleted, Message = "Multiple files deleted successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error deleting multiple files");
            return new JsonModel { data = new object(), Message = $"Error deleting multiple files: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public async Task<JsonModel> GetStorageInfoAsync()
    {
        try
        {
            ...
            return new JsonModel { data = storageInfo, Message = "Storage info retrieved successfully", StatusCode = 200 };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error getting storage info");
            return new JsonModel { data = new object(), Message = $"Error getting storage info: {ex.Message}", StatusCode = 500 };
        }
    }
    ...
    public Task<JsonModel> CleanupExpiredFilesAsync()
    {
        ...
        return Task.FromResult(JsonModel.SuccessResponse(true, "Cleanup completed successfully"));
    }
    ...
    public Task<JsonModel> ArchiveOldFilesAsync(string sourcePath, string archivePath, TimeSpan ageThreshold)
    {
        ...
        // For now, return success
        return Task.FromResult(JsonModel.SuccessResponse(true, "Archiving completed successfully"));
    }
